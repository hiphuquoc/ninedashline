@php($mapsAria = $ancientMapsLangPrefix ?? 'ancient_maps')
  ...
  @foreach ($ancientChineseMaps as $mi => $map)
    ...
    >
      ...
      <div class="ancient-map-entry__content">
        @if (! empty($map['images']))
          @php($mapImageCount = count($map['images']))
          <div class="ancient-map-entry__gallery" data-count="{{ $mapImageCount }}">
            @foreach ($map['images'] as $img)
              <figure class="ancient-map-entry__figure">
                <button
                  ...
                  data-title="{{ e($img['caption'] !== '' ? $img['caption'] : $map['title']) }}"
                  aria-label="{{ t('lightbox_aria') }}: {{ $img['caption'] !== '' ? $img['caption'] : $map['title'] }}"
                >
                  <img
                    src="{{ $img['src'] }}"
                    alt=""
                ...
                @if ($img['caption'] !== '')
                  <figcaption class="ancient-map-entry__caption">{{ $img['caption'] }}</figcaption>
                @endif
              </figure>
            @endforeach
          </div>
